...
本文件提供 Raidcore Nexus 審核人員一份可由原始碼直接驗證的 Upgrade Value 行為與審核範圍摘要。內容對應外掛版本 `1.0.4.0`，並使用專案內附的 Nexus API 版本 `6` 建置。
...
6. 在 ImGui 表格中顯示結果、提供三態位置排序，並依使用者選擇的估價方式與門檻計算建議。
